...
- **Numbering: one file per version, always after `main`.** `supabase db push` pairs local files with production's `supabase_migrations.schema_migrations` by version only — the digits before the first `_` — never by filename. Checked on 2026-09-15 with the CLI version CI uses (2.117.0) against a throwaway database: two files with the same version make the push fail on `schema_migrations_pkey` (while `--dry-run` still exits 0); a version lower than the last applied one is rejected, because the deploy deliberately omits `--include-all`. Two cases pass silently: a `.sql` whose name has no version is skipped with only a warning, and **renumbering a migration that was already applied** makes the push treat whichever file kept the old version as applied — it never runs — and re-run the renamed one under the new number. Parallel branches collide on numbers routinely (on 2026-09-14, 0047 was taken in three branches at once), so:
  - Number a migration against `origin/main` **at merge time**, not when branching. If `main` moved past it, renumber the migration your PR adds before merging.
  - Never renumber a migration that is already on `main`. If a duplicate does reach `main`, renumber the one whose name production has *not* recorded in `schema_migrations` (it stores `name` next to `version`).
  - `scripts/check-migration-versions.mjs` enforces this in the `test` job, before dependencies are installed or Supabase starts: on every run, no `.sql` file without a version and no shared versions; on pull requests, every migration the PR adds must be newer than the highest on `origin/main`. Locally: `git fetch origin main && node scripts/check-migration-versions.mjs --base origin/main`.
