...
- **Migrations are applied by hand, and must be applied BEFORE merging** anything that adds one — Vercel deploys `main` automatically, so a merge without the migration puts new code on an old schema (that outage happened on 2026-08-19). Run `supabase db push --dry-run --linked` to preview, then `supabase db push --linked`. The `deploy` job in `.github/workflows/ci.yml` was built to automate this ordering but is currently **disabled**: the CI build produced a broken artifact — see the comment block on that job. `deploy/production-migration.sql` is obsolete and kept only as a no-op note.
