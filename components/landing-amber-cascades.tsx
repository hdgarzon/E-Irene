  ...
  function render(timestamp: number) {
    ...
    for (const col of columns) {
      ...
      for (let j = 0; j < col.length; j++) {
        ...
        if (j === 0) {
          ctx.shadowColor = "rgba(30, 130, 230, 0.5)";
          ctx.shadowBlur = 8;
        }
    ...
    waterGrad.addColorStop(0, "rgba(224, 236, 247, 0.6)");
    waterGrad.addColorStop(1, "rgba(247, 250, 252, 0.95)");
    ...
    ctx.strokeStyle = "rgba(30, 120, 220, 0.35)";
    ctx.lineWidth = 1.5;
    ctx.stroke();
    ...
    for (const r of ripples) {
      ...
      for (let ring = 0; ring < 3; ring++) {
        ...
        ctx.strokeStyle = `rgba(30, 120, 220, ${alpha * (1 - ring * 0.3)})`;
...
/**
 * Animación de fondo del hero: columnas de símbolos cayendo que "chocan"
 * contra una superficie de agua simulada, con física de olas y ondas al
 * hacer click/tap. Respeta prefers-reduced-motion.
 */
export function LandingAmberCascades() {
