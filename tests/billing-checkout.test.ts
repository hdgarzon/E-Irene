...
describe("createTranscriptionPackCheckout", () => {
  const CLINIC = "6550747c-13a0-4cfb-a88a-b1cb9bb99952";
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () => ({
        ok: true,
        status: 200,
        text: async () => JSON.stringify({ data: { id: "test_pack123", active: true } }),
      })) as unknown as typeof fetch,
    );
  });
  ...
  it("cobra el precio de la bolsa con un link de 24 horas y registra la compra", async () => {
    const now = new Date("2026-09-15T15:00:00Z");
    const result = await createTranscriptionPackCheckout({
      clinicId: CLINIC,
      plan: "pro",
      cycleEnd: "2026-10-01T05:00:00.000Z",
      redirectUrl: "https://e-irene.co/settings/plan",
      now,
    });

    const body = JSON.parse((fetch as unknown as ReturnType<typeof vi.fn>).mock.calls[0][1].body);
    expect(body.amount_in_cents).toBe(2_500_000);
    expect(body.expires_at).toBe("2026-09-16T15:00:00");
    expect(body.single_use).toBe(true);

    // Nunca se lee como la compra de un plan, que activaría o reiniciaría la suscripción.
    expect(result.reference).toMatch(new RegExp(`^transcriptionpack-${CLINIC}-\\d+$`));
    expect(parseBillingReference(result.reference)).toBeNull();

    expect(recordCheckout).toHaveBeenCalledWith(
      expect.objectContaining({
        paymentLinkId: "test_pack123",
        clinicId: CLINIC,
